...
for($i = 0; $i < 3000; $i++){
  ...
  $bytes = \Eventsd\Eventsd::trigger("loadtest", json_encode(array('seq' => $i, 'uuid' => \FourOneOne\ActiveRecord\UUID::v4(), 'url'=>$_SERVER['REQUEST_URI'], 'delay' => $delay)));
  echo "Sent Test {$i}, {$bytes} bytes\n";
  $bytes_total = $bytes_total + $bytes;
}
echo "OK.\n";
echo "Complete in ". $delay . " seconds.\n".number_format($bytes_total/1024,2)." KiloBytes";